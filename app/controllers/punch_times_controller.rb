class PunchTimesController < ApplicationController
  ...
  def create
    ...
    @punch_time.project_id = params[:project][:project_id] if params[:project_id]
    @punch_time.organization_id = params[:organization][:organization_id] if params[:organization]
    if @punch_time.save
